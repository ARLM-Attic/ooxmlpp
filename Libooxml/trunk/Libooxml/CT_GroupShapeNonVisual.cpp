...
CT_GroupShapeNonVisual::CT_GroupShapeNonVisual(xercesc_3_1::DOMNodeList *nodelist, xercesc_3_1::DOMNamedNodeMap *attributes) {
	nvPr = NULL;
	cNvPr = NULL;
	cNvSpPr = NULL;
	for (int i = 0; i < nodelist->getLength();++i) {
		...
		if (wcscmp(nodelist->item(i)->getLocalName(),L"cNvSpPr") == 0) {
			cNvSpPr = new drawingml::main::CT_NonVisualGroupDrawingShapeProps(nodelist->item(i)->getChildNodes(),nodelist->item(i)->getAttributes());
		}
	}
}
